...
import AddSeatBatchDialog from './components/AddSeatBatchDialog';
import EditSeatBatchDialog from './components/EditSeatBatchDialog';
...
export default function SeatBatchPage() {
  ...
  const [data, setData] = useState<SeatBatch[]>([]);
  const [currentData, setCurrentData] = useState<SeatBatch>();

  const [openViewDialog, setOpenViewDialog] = useState(false);
  const [openEditDialog, setOpenEditDialog] = useState(false);
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
  const [openGenerateCodeDialog, setOpenGenerateCodeDialog] = useState(false);
  ...
  const fetchSeatBatches = useCallback(async () => {
    try {
      const response = await api.get('/seat-batches');
      setData(response.data);
    } catch {
      toast.error('Error fetching seat batches');
    }
  }, []);
  ...
  const refreshState = () => {
    ...
    setOpenGenerateCodeDialog(false);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.delete(`/seat-batches/${currentData._id}`);

      refreshState();
      toast.success('Successfully deleted seat batch');
    } catch (error) {
      toast.error('Error deleting seat batch');
    }
  };
  ...
  const handleGenerateCode = (id: string) => {
    ...
    if (data) {
      setCurrentData(data);
      setOpenGenerateCodeDialog(true);
    }
  };
  ...
  return (
    <div className="w-full">
      ...
      <GenerateSeatCodeDialog
        open={openGenerateCodeDialog}
        onOpenChange={setOpenGenerateCodeDialog}
        seatBatch={currentData}
        onSuccess={refreshState}
      />
    </div>
